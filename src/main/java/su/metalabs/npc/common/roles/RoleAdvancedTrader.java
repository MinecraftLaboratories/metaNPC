...
public class RoleAdvancedTrader extends RoleInterface {
    private boolean ignoreMeta = false;
    private boolean ignoreNBT = false;

    @Getter
    @Setter
    private boolean drawFastSell = false;
    private final TradeSlot[] slots = new TradeSlot[24];

    public RoleAdvancedTrader(EntityNPCInterface npc) {
        super(npc);
        for (int i = 0; i < 24; ++i)
    ...
    public NBTTagCompound writeToNBT(NBTTagCompound nbtTagCompound) {
        ...
        data.setBoolean("drawFastSell", drawFastSell);
        NBTTagList list = new NBTTagList();
        for (TradeSlot slot : slots) {
            list.appendTag(slot.writeNBT());
        }
    ...
    public void readFromNBT(NBTTagCompound nbtTagCompound) {
        ...
        drawFastSell = data.getBoolean("drawFastSell");
    ...
    public void interact(EntityPlayer entityPlayer) {
        ...
        entityPlayer.openGui(MetaNpc.instance, 0, npc.getEntityWorld(), npc.getPlayerCoordinates().posX, npc.getPlayerCoordinates().posY, npc.getPlayerCoordinates().posZ);
    ...
    public void onTryBuy(int slot, EntityPlayer player) {
        ...
        if (slot >= 0 && slot < 24) {
            ...
            TradeSlot tradeSlot = getSlot(slot);
            for (int i = 0; i < 2; ++i) {
                if (tradeSlot.getSlotIn(i) != null && tradeSlot.getSlotIn(i).execute(player, this)) {
                    continue;
                } else if (tradeSlot.getSlotIn(i) != null) {
                    player.inventory.readFromNBT(list);
                    if (i == 0)
                        return;
                    else
                        payback(player, tradeSlot.getSlotIn(0));
                    return;
                }
            }
            for (int i = 0; i < 2; ++i) {
                if (tradeSlot.getSlotOut(i) != null) {
                    tradeSlot.getSlotOut(i).execute(player, this);
                }
            }

            if(tradeSlot.getSlotXp() != null) {
                tradeSlot.getSlotXp().execute(player, this);
            }
        }
    }
    ...
    public static class TradeSlot {
        ...
        public String getMessageFromSlot(EntityPlayer player, RoleAdvancedTrader role) {
    ...
    public static class TradeSlotXp implements IContentSlot {
        ...
        public boolean execute(EntityPlayer player, RoleAdvancedTrader role) {
    ...
    public static class TradeSlotIn implements IContentSlot {
        ...
        public void calculated() {
            if (data != null && data instanceof ItemStack && ((ItemStack)data).getItem() == CommonProxy.setupItem) {
                ItemStack t = (ItemStack) data;
                if (ItemSetup.getRub(t) != -1) {
                    type = TradeType.RUB;
                    data = ItemSetup.getRub(t);
                    return;
                }
                if (ItemSetup.getGem(t) != -1) {
                    type = TradeType.GEMS;
                    data = ItemSetup.getGem(t);
                    return;
                }
                if (ItemSetup.getGold(t) != -1) {
                    type = TradeType.COINS;
                    data = ItemSetup.getGold(t);
                    return;
                }
        ...
        public ItemStack getStackToGM() {
            switch (type) {
                ...
                case RUB:
                case COINS:
                    ItemStack st = new ItemStack(CommonProxy.setupItem);
    ...
    public static class TradeSlotOut implements IContentSlot {
        ...
        public void execute(EntityPlayer player, RoleAdvancedTrader role) {
            switch (type) {
                case STACK: {
